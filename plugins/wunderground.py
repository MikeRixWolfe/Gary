...
wunder_url = "http://api.wunderground.com/api/{}/{}/geolookup/conditions/q/{}.json"
cards = {
    0: "N",
    22.5: "NNE",
    45: "NE",
...
def weather(inp, say=None, api_key=None):
    ...
    try:
        weather = http.get_json(wunder_url.format(api_key, 'forecast', http.quote(inp)))
        if weather.get('current_observation', None) is None and len(weather['response'].get('results', [])) > 0:
            weather = http.get_json(wunder_url.format(api_key, 'forecast', weather['response']['results'][0]['zmw']))
    except:
...
def forecast(inp, say=None, api_key=None):
    ...
    try:
        weather = http.get_json(wunder_url.format(api_key, 'forecast', http.quote_plus(inp)))
        if weather.get('current_observation', None) is None and len(weather['response'].get('results', [])) > 0:
            weather = http.get_json(wunder_url.format(api_key, 'forecast', weather['response']['results'][0]['zmw']))
    except:
        return "Weather Underground API error, please try again in a few minutes."
...
def hourly(inp, say=None, api_key=None):
    ...
    try:
        weather = http.get_json(wunder_url.format(api_key, 'hourly', http.quote_plus(inp)))
        if weather.get('hourly_forecast', None) is None and len(weather['response'].get('results', [])) > 0:
            weather = http.get_json(wunder_url.format(api_key, 'hourly', weather['response']['results'][0]['zmw']))
    except:
        return "Weather Underground API error, please try again in a few minutes."
    try:
        say("\x02{location[city]}, {location[state]}\x0F: ".format(**weather) +
