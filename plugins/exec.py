...
def python(inp, say=None):
    ...
    return re.sub(" +"," "," ".join(out.split(" ")).replace("\n"," ")) or "No output."
...
def ruby(inp, say=None):
    ...
    try:
        ...
        html = http.get_xml("http://codepad.org/", query_params=params, get_method="POST")
...
def ply(inp):
    ...
    try:
        return subprocess.check_output(["python", "-c", inp], stderr=subprocess.STDOUT)
