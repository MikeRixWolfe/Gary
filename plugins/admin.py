...
def unmutechan(inp, conn=None, chan=None, notice=None):
    """unmute [channel] - Makes the bot unmute a channel. If [channel] is blank the bot will mute the channel the command was used in."""
...
def rsay(inp, conn=None, chan=None):
    """rsay [channel] <message> - Makes the bot say <message> in [channel]. If [channel] is blank the bot will the command was used in."""
    target, msg = inp.split(' ', 1) if inp[0] == '#' else (chan, inp)
...
def pm(inp, conn=None, chan=None):
    """pm <user> <message> - Makes the bot send <message> message <user>. If [channel] is blank the bot will say the <message> in the channel the command was used in."""
    inp = inp.split(" ")
    if len(inp) > 1:
        message = " ".join(inp[1:])
        out = "PRIVMSG {} :{}".format(inp[0], message)
