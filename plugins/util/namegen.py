...
def artygen(n=1):
    ...
    with open('plugins/data/pq_artifact_dict.json') as f:
        arty = json.load(f)
    artifact = []
    for k in range(n):
        artifact_pieces = []
...
def monster_gen(n=1):
    ...
    with open('plugins/data/pq_monster_description_dict.json') as f:
    ...
    for kk in range(n):
        ...
        if random.randint(0, 1):
            ...
            if z > 1:
                ...
                pr = random.choice(monster_description['prey'])
...
def taverngen(n=1, longname=False):
    ...
    with open('plugins/data/pq_taverns_tuple.json') as f:
        taverns = json.load(f)
    tavern = []
    for kk in range(n):
...
def riddlegen(n=1):
    ...
    with open('plugins/data/pq_riddle_dict.json') as f:
        riddles = json.load(f)
    riddle = []
    for i in range(n):
...
def godgen(n=1):
    ...
    with open('plugins/data/pq_gods_tuple.json') as f:
        gods = json.load(f)
    god = random.choice([random.sample(gods, n) for j in range(6)])
    if isinstance(god, list) and len(god) == 1:
        return god[0]
    return god
