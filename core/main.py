...
class Input(dict):
    def __init__(self, conn, raw, prefix, command, params,
        ...
        def action(chan, message):
            conn.ctcp(chan, "ACTION", message)

        dict.__init__(self, conn=conn, raw=raw, prefix=prefix, command=command,
                    params=params, nick=nick, user=user, host=host,
                    paraml=paraml, msg=msg, server=conn.server, chan=chan,
                    notice=notice, say=say, reply=reply, pm=pm, bot=bot,
                    me=me, action=action, set_nick=set_nick, lastparam=paraml[-1])
...
def dispatch(input, kind, func, args, autohelp=False):
    ...
    if hasattr(func, '_apikey'):
        keys = {}
        for _key in func._apikey.split(','):
            key = bot.config.get('api_keys', {}).get(_key, None)
            if key is None:
        ...
        if len(keys) == 1:
            input.api_key = keys[keys.keys()[0]]
        else:
            input.api_key = keys
...
def main(conn, out):
    ...
    if inp.command == 'PRIVMSG':
        # COMMANDS
        cmd_prefix = re.escape(bot.config.get("prefix", "."))
